...
<AccordionGroup>
  ...
  <Accordion icon="bolt" title="Performance Analysis">
    Detects runtime performance antipatterns:
    - **N+1 Queries**: Repeated database or ORM queries inside loops
    - **Inefficient List Operations**: `list()` wrapping comprehensions, redundant iterations
    - **Inefficient String Concatenation**: `+=` string building inside loops (use `join` instead)
    - **Unnecessary Iterations**: Patterns that can be replaced with comprehensions or built-ins
    - **Redundant `list()` Calls**: Wrapping a list comprehension in `list()`

    ```python
    # ❌ Performance issues detected
    for user in users:
        profile = db.query(Profile).filter_by(user_id=user.id).first()  # N+1 query
