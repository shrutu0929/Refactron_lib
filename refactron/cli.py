...
def main(ctx: click.Context) -> None:
    ...
    # But SKIP if it's just --help or --version
    ...
    should_show_animation = (
        (
            ...
            or (ctx.invoked_subcommand not in exempt_commands and not is_authenticated)
        )
        and not is_help
        and not is_version
    )
    ...
    if (
        ctx.invoked_subcommand not in exempt_commands
...
@click.option(
    ...
    "-a",
    type=click.Choice(
        ["accepted", "rejected", "ignored", "accepted_as_smell"], case_sensitive=False
    ),
    required=True,
    help="Feedback action: accepted, rejected, ignored, or accepted_as_smell",
)
@click.option(
    "--reason",
    "-r",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Include issues suppressed by AI in the report",
)
def report(
    target: str,
    config: Optional[str],
    ...
    cfg.include_suppressed = include_suppressed
